...
void PitchShift(float pitch_factor, int* inbuf, int* outbuf) {
  ...
  if (change_direction == 1) {
    ...
    // initialize ch buffers to 0 except first spot where input will be shifted
    // in
  ...
  for (m = 0; m < BUFF_SIZE; m++) {
    if (((bufferAptr_float + 1) >= 0) &
        ((bufferAptr_float + 1) <= BUFFER_DEPTH - 1))
    ...
    if (((bufferBptr_float + 1) >= 0) &
        ((bufferBptr_float + 1) <= BUFFER_DEPTH - 1))
    ...
    // Compute Channel Outputs & system output  Simplified model for now w/o
    // interpoloations.
    ...
    if ((pitch_shift_up == 1) &
        (((delaya <= DELAY_DEPTH_UP)) || ((delayb <= DELAY_DEPTH_UP))) &
        (crossfading != 1)) {
      crossfading = 1;
      ...
      if ((pitch_shift_up == 0) &
          (((delaya >= DELAY_DEPTH_DOWN)) || ((delayb >= DELAY_DEPTH_DOWN))) &
          (crossfading != 1)) {
        crossfading = 1;
        crossfadingAB = !(crossfadingAB);
        q = 1;
    ...
    if (pitch_shift_up) {
      ...
      if (Gb != 0) bufferBptr_float = bufferBptr_float - pitch_shift_rate;
    } else {
      if (Ga != 0)  // only change pointers for active channels
        bufferAptr_float = bufferAptr_float + pitch_shift_rate;
      if (Gb != 0) bufferBptr_float = bufferBptr_float + pitch_shift_rate;
