...
// -------------------------------------------------------
#define IN_SAMPLES 1000
#define BUFFER_SIZE 64
short inputBuffer[IN_SAMPLES];
short outputBuffer[IN_SAMPLES];
int leftIndex = 0;
int outsamples = 0;
// -------------------------------------------------------
static void handle_leftready_interrupt_test(void* context, alt_u32 id) {
	 ...
	 *leftreadyptr = IORD_ALTERA_AVALON_PIO_EDGE_CAP(LEFTREADY_BASE);
	 ...
	 if (leftIndex == BUFFER_SIZE){
		 outsamples = sola(outputBuffer, inputBuffer, IN_SAMPLES);
	 }


	 leftIndex = (leftIndex + 1) % IN_SAMPLES;
}
